...
import { fetchReferrals, fetchReferralDetail } from "@/lib/referrals";
...
const TOOLS: Anthropic.Tool[] = [
  {
    name: "list_runs",
    description:
      "List all patient referrals with their statuses, patient names, and dates. Returns up to 50 recent referrals.",
    input_schema: {
  ...
  },
  {
    name: "get_run",
    description:
      "Get full details of a specific patient referral by its run ID. Includes all extracted patient data, specialist match, and referral notice.",
    input_schema: {
      type: "object" as const,
      properties: {
        run_id: { type: "string", description: "The run ID of the referral to retrieve" },
      },
      required: ["run_id"],
    },
  },
  {
    name: "get_automation",
    description:
      "Get details about the Patient Referral Intake automation, including its code, connections, and description.",
    input_schema: {
      type: "object" as const,
      properties: {},
      required: [],
...
async function executeTool(name: string, input: Record<string, unknown>): Promise<string> {
  switch (name) {
    ...
    case "get_run": {
      ...
      const detail = await fetchReferralDetail(runId);
